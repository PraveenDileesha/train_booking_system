...
-- ---------- Route Versions ----------
-- Immutable snapshot of a route's station list.
-- Editing a route creates a new version rather than mutating an existing one.
...
-- ---------- Trip Seats ----------
-- A seat as it exists on a specific trip.
-- Only populated for seats whose coach is actually attached to that trip via trip_coaches.
...
-- Prevents two confirmed bookings on the same trip_seat from covering overlapping ranges.
-- Deferrable so it can be checked once at commit time during multi-row updates (e.g. a new route version).
